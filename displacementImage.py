...
path2 = os.path.join(stasm.DATADIR, '/home/shivani/Pictures/cohn-kanade-images-extracted/S037_001_00000020.png')
...
def drawArrowLine(image, p, q, color, thickness, line_type, shift):
	arrow_magnitude = 0.2* math.sqrt(((p[0]-q[0])*(p[0]-q[0]))+((p[1]-q[1])*(p[1]-q[1])))
...
if len(landmarks) == 0:
	...
	for origin, dest in zip(landmarks,landmarks2):
		...
		#cv2.putText(img,"x",(x,y), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(250,255,0))
